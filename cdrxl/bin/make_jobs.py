...
if __name__ == '__main__':
    ...
    ''')
    argparser.add_argument('paths', nargs='+', help='Path(s) to CDRXL config file(s).')
    argparser.add_argument('-j', '--jobtype', default='train', help='Type of job, one of "train", "eval".')
    ...
    argparser.add_argument('-m', '--memory', type=int, default=16, help='Number of GB of memory to request')
    ...
    for path in [x for x in paths if x.endswith('.yml')]:
        ...
        with open(filename, 'w') as f:
            ...
            job_str = wrapper % ('python3 -m cdrxl.bin.%s %s' % (jobtype, path))
            f.write(job_str)
